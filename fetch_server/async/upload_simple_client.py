...
from uploadclient import HTTPRequest, HTTPResponse, HTTPError, AsyncHTTPClient, main
...
class UploadAsyncHTTPClient(AsyncHTTPClient):
    ...
    def _process_queue(self):
        with stack_context.NullContext():
            while self.queue and len(self.active) < self.max_clients:
                ...
                if request.url in self.keep_alive and len(self.keep_alive[request.url]) > 1:
                    conn, length = self.keep_alive[request.url]
                    length = int(length)
                    if not isinstance(conn, _HTTPConnection):
                        fileno = int(conn)
                        key = object()
                        self.active[key] = (request, callback)
                        conn = _HTTPConnection(self.io_loop, self, request,
                                     functools.partial(self._on_fetch_complete,key, callback), 
                                     fileno=fileno)

                        self.keep_alive[request.url] = conn

                    if request.body:
                        conn.write_body(request.body, length, 
                                        functools.partial(self._on_fetch_complete,
                                                          'UPLOAD', callback)
                                        )
                    else:
                        conn.read_body(length, functools.partial(self._on_fetch_complete,
                                                          'DOWNLOAD', callback))
                else:
                    key = object()
                    self.active[key] = (request, callback)
    ...
    def _on_fetch_complete(self, key, callback, url, response, fileno='', length=''):
        if key in self.active:
            del self.active[key] 
        if url in self.keep_alive:
            del self.keep_alive[url]
        fileno = str(fileno).decode("utf-8")
        length = str(length).decode("utf-8")

        callback(response, fileno, length)
        self._process_queue()
...
class _HTTPConnection(object):
    ...
    def __init__(self, io_loop, client, request, callback, fileno=None):
        ...
        self.body_length = 0
        self.body_left = 0
        self.datalimit = int(0x100000)
        self.alive_fd = fileno
    ...
    def _on_timeout(self):
        ...
        if self.callback is not None:
            self.callback(self.request.url, HTTPResponse(self.request, 599,
    ...
    def _on_connect(self, parsed):
        ...
        if has_body:
            #self.body_length = len(self.request.body)
            if len(self.request.body) <> int(self.request.headers['Content-Length']):
                self.write_body(self.request.body, 0, self.callback)
    ...
    def write_body(self, body, length, callback):
        # Update time out
        self.start_time = time.time()
        if self._timeout is not None:
            self.io_loop.remove_timeout(self._timeout)
            self._timeout = None
        self._timeout = self.io_loop.add_timeout(
            self.start_time + self.request_timeout,
            self._on_timeout)
        ...
        try:
            self.stream.write(body, self.complete_write)
        except:
            self._on_close()
    ...
    def complete_write(self):
        ...
        if self.body_length >= int(self.request.headers['Content-Length']):
            self.complete_body()
        elif self.body_length <> int(self.request.headers['Content-Length']):
            if self.callback is not None:
                self.callback(None, HTTPResponse(self.request, 593,
                                      headers=self.headers,
                                      error=HTTPError(593, "Upload continue")),
                              fileno = self.stream.socket.fileno(),
    ...
    def cleanup(self):
        try:
            ...
            if self.callback is not None:
                ...
                self.callback = None
                callback(self.request.url, HTTPResponse(self.request, 599, error=e, headers=self.headers))

    def _on_close(self):
        if self.callback is not None:
            callback = self.callback
            self.callback = None
            callback(self.request.url, HTTPResponse(self.request, 599,
                                  headers=self.headers,
    ...
    def _on_headers(self, data):
        ...
        if self.headers.get("Transfer-Encoding") == "chunked":
            ...
            # Hack by zay
            self.body_left = self.body_length = int(self.headers["Content-Length"])
            if self.body_length > int(self.datalimit):
                self.read_body(self.body_left,self.callback)
                return self
    ...
    def read_body(self, length, callback):
        # Update time out
        self.start_time = time.time()
        if self._timeout is not None:
            self.io_loop.remove_timeout(self._timeout)
        ...
        if self.body_left >= self.datalimit:
            length = self.datalimit
        else :
            length = self.body_left
        
        self.body_left -= length
        if length > 0:
            self.stream._keep_read = True
            self.stream.read_bytes(length,
                                   self._end_read)
        else:
            self.complete_read_body()
        
        
    def _end_read(self, data):        
        if self._timeout is not None:
            self.io_loop.remove_timeout(self._timeout)
            self._timeout = None
        ...
        response = HTTPResponse(original_request,
                                592, headers=self.headers,
                                buffer=buffer,
                                effective_url=self.request.url)
        if callable(self.callback):
            self.callback(None, response, 
                          fileno=self.stream.socket.fileno(), 
                          length=self.body_left) 
            self.callback = None
        

    def complete_read_body(self) :
        self.stream._keep_read = False
        if callable(self.callback):
            self.callback(self.request.url, HTTPResponse(self.request, 594,
                                      headers=self.headers,
                                      error=HTTPError(594, "Download completed!")))
            self.callback = None
    ...
    def _on_body(self, data):
        ...
        response = HTTPResponse(original_request,
                                ...
                                effective_url=self.request.url)
        if callable(self.callback):
            self.callback(self.request.url, response)
        self.callback = None
...
if __name__ == "__main__":
    AsyncHTTPClient.configure(UploadAsyncHTTPClient)
    main()
